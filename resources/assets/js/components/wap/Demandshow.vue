        ...
        <section class="content">
            <div class="wrap product">
                <div class="info">
                    ...
                    <div class="detail">
                        <h2>{{product.title}}</h2>
                        <span class="price">数量：{{product.quantity}}<em v-if="product.unit===1">只</em><em v-if="product.unit===2">个</em><em v-if="product.unit===3">扎</em><em v-if="product.unit===4">袋</em><em v-if="product.unit===5">箱</em></span>
...
<script type="text/babel">
    export default {
        ...
        methods: {
            productShow() {
                var _this = this
                this.$http.get('/api/demands/'+ this.id)
                        .then(response => {
