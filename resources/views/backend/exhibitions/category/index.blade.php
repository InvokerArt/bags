        ...
        <div class="tab-content">
            <div class="row">
                ...
                <div class="col-xs-10 category-new">
                    {!! Form::model($category,['route' => [env('APP_BACKEND_PREFIX').'.exhibitions.categories.update', $category->id], 'id' => 'category','class'=>'form-horizontal','method' => 'PATCH','enctype'=>'multipart/form-data']) !!}
    ...
    <script>
        $(function () {
            ...
            .on('delete_node.jstree', function (e, data) {
                $.post("/{{ env('APP_BACKEND_PREFIX') }}/exhibitions/categories/"+data.node.id, { '_method' : 'delete', '_token' : '{{ csrf_token() }}' })
                        .fail(function () {
            ...
            .on('create_node.jstree', function (e, data) {
                $.post("{{ route(env('APP_BACKEND_PREFIX').'.exhibitions.categories.store') }}", { 'id' : data.node.parent, 'name' : data.node.text, '_token' : '{{ csrf_token() }}' })
                        .done(function (d) {
            ...
            .on('rename_node.jstree', function (e, data) {
                $.post("{{ route(env('APP_BACKEND_PREFIX').'.exhibitions.categories.rename') }}", { 'id' : data.node.id, 'name' : data.text, '_token' : '{{ csrf_token() }}' })
                        .fail(function () {
                            data.instance.refresh();
                        })
                        .done(function () {
            ...
            .on('move_node.jstree', function (e, data) {
                $.post("{{ route(env('APP_BACKEND_PREFIX').'.exhibitions.categories.move') }}", { 'id' : data.node.id, 'parent' : data.parent, '_token' : '{{ csrf_token() }}' })
                        .fail(function () {
                            data.instance.refresh();
                        })
                        .done(function () {
